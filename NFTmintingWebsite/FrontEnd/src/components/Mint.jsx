...
const CryptoPigsAddress = '0xE4A35F7c60A559A308286B5B2c1Ee36fb8762f23';
...
const Mint = ({ wallet, setWallet }) => {
		...
		try {
			if (ethereum) {
				...
				if (ethereum && Contract) {
					...
					const PolyogonID = '0x13881';
					if (currentChainID === PolyogonID) {
						if (mintAmt >= 1 && mintAmt <= 10) {
							...
							setMintAmt('');
							setMessage(
								`NFTs minted...Have a look at Ur wallet@ :https://testnets.opensea.io/`
							);
	...
	return (
		<MintContainer>
			<h1>
				Please Connect to{' '}
				<span style={{ borderBottom: '5px solid rgba(219, 229, 237, 0.878)' }}>
					Polygon TestNet
				</span>{' '}
				to Mint NFTs
			</h1>
